...
if (DO_SBOM === "true") {
    logger.info("Generating SBOM for zeus-install ...")

    // get the version
    // the version from zeus-install comes from the recipe json file

    let zeus_install_version = META.version
    ...
    _cdx_sbom.components.add(
        new CDX.Models.Component(
            ...
            {
                version: `${zeus_install_version}`,
                cpe: `cpe:2.3:*:*:zeus-install:${zeus_install_version}:*:*:*:*:*:*:*`,
                purl: PackageURL.fromString(`pkg:generic/zeus-install@${zeus_install_version}`)
            }
        )
    )
    ...
    logger.success("SBOM generation for zeus-install completed successfully.")
}
